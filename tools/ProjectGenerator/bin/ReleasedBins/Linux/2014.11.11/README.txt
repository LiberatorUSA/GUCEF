...
This is a release build created on 11th of November 2014.
...
- Support for multiple generator backends:
     ...
     - Android make
     - Premake4
- Extensible via plugins to allow support for more exotic scenarios.
  Plugin provided to filter dependencies using the Depends tool
  Plugin provided to generate initial ModuleInfo from Visual Studio projects 
  to aid in migration of Visual Studio only projects    
...
The support generators are:
  ...
  - "premake4"
       Generates "premake4.lua" files which are used by Premake4 to generate project 
       files for specific IDEs and such similar to CMake     
...
You can configure everything using an ini style configuration file.
An example configuration file should be included with the binaries.

Command line paramaters are applied after the loading of the config file
and as such can be used to override the file based config settings.

The tool always checks for a "ProjectGenerator.ini" file in the tool directory
if no ini file is found the current directory is checked.
...
- 11th November 2014 :
     - Linux build using latest code. A lot of unannounced features were added since the last binary release
     - Restructured the code a bit. The main workhorse is now the ProjectGen module with minimal functionality housed in
       the enveloping application module "ProjectGenerator". This and other refactoring was aimed at proving plugin support
       in line with the rest of the GU codebase.
     - Added importer plugin for Visual Studio vcxproj projects. Aimed at making it easier to transition large projects.
     - Added plugin which filters the located modules based on output from the tool Depends. This plugin is aimed at
       making it easier to define a project based on a large codebase where it is unclear what the minimum subset needed is.
     - Although part of the codebase for some time it was never officially announced: Premake4 support was added as a generator.
- 22'nd January 2013 :
