...
{GUCEF_TRACE;
    ...
    const CORE::CString bootstrapConfigFile = "UdpTransformer_bootstrap_d.ini";
    const CORE::CString configFile = "UdpTransformer_d.ini";
    #else
    const CORE::CString bootstrapConfigFile = "UdpTransformer_bootstrap.ini";
    const CORE::CString configFile = "UdpTransformer.ini";
    #endif
 ...
 */
//GUCEF_OSMAIN_BEGIN
GUCEF_OSSERVICEMAIN_BEGIN( "UdpTransformer" )
{GUCEF_TRACE;
    ...
    CORE::CString logFilename = CORE::CombinePath( outputDir, "UdpTransformer_log.txt" );
    ...
    if ( !udpTransformer.LoadConfig( keyValueList, *globalConfig ) )
    {
        delete globalConfig;
        GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "UdpTransformer: Exiting because LoadConfig failed" );
        return -1;
    }
    delete globalConfig;

    if ( !udpTransformer.Start() )
    {
        GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "UdpTransformer: Failed to Start()" );
        return -2;
    }
