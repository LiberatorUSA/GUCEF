...
MAINCMAKE=$SRCROOTDIR/projects/CMake/targets/GUCEF_exe_HDFiller
NIXMAKE_OUTPUTDIR="$OUTPUTDIR/UnixMakeDbgClang_HDFiller"
TOOLCHAINFILE="$SRCROOTDIR/projects/CMake/CMakeClang64ToolChain.txt"

echo "*** Invoking CMake ***"
echo "Output dir for this CMake target = $NIXMAKE_OUTPUTDIR"
echo "CMake will use source root: $MAINCMAKE"
echo "CMake will use toolchain file: $TOOLCHAINFILE"
cmake -D_CMAKE_TOOLCHAIN_PREFIX=llvm- -DBUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=debug -DCMAKE_TOOLCHAIN_FILE=$TOOLCHAINFILE -G"Unix Makefiles" -H"$MAINCMAKE" -B$NIXMAKE_OUTPUTDIR
