...
{GUCEF_TRACE;
    ...
    CORE::CString logFilename = CORE::CombinePath( outputDir, "ProcessMetrics_log.txt" );
    ...
    {
        delete globalConfig;
        GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "ProcessMetrics: Exiting because LoadConfig failed" );
        return -1;
    }
    delete globalConfig;

    if ( !processMetrics.Start() )
    {
        GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "ProcessMetrics: Failed to Start()" );
        return -2;
    }
