...
{GUCEF_TRACE;
    ...
    const CORE::CString configFile = "PackageUpgrader_d.ini";
    #else
    const CORE::CString configFile = "PackageUpgrader.ini";
    #endif
 ...
 */
GUCEF_OSMAIN_BEGIN
//GUCEF_OSSERVICEMAIN_BEGIN( "PackageUpgrader" )
{GUCEF_TRACE;
    ...
    CORE::CString logFilename = CORE::CombinePath( outputDir, "PackageUpgrader_log.txt" );
    ...
    PackageUpgrader udp2ViaTcp;
    if ( !udp2ViaTcp.LoadConfig( keyValueList, *globalConfig ) )
    {
        delete globalConfig;
        GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "PackageUpgrader: Exiting because LoadConfig failed" );
        return -1;
    }
    delete globalConfig;
    ...
    if ( !udp2ViaTcp.Start() )
    {
        GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "PackageUpgrader: Failed to Start()" );
        return -2;
    }
