...
CTestPubSubClientTopic::StorageToPubSubRequest::StorageToPubSubRequest( void )
    : startDt()
    , endDt()
...
CTestPubSubClientTopic::StorageToPubSubRequest::StorageToPubSubRequest( const CORE::CDateTime& startDt, const CORE::CDateTime& endDt )
    : startDt( startDt )
    , endDt( endDt )
    , vfsPubSubMsgContainersToPush()
{GUCEF_TRACE;
...
CTestPubSubClientTopic::StorageToPubSubRequest::StorageToPubSubRequest( const StorageToPubSubRequest& src )
    : startDt( src.startDt )
    , endDt( src.endDt )
...
const CORE::CString&
CTestPubSubClientTopic::StorageToPubSubRequest::GetClassTypeName( void ) const
{GUCEF_TRACE;
...
CORE::CString
CTestPubSubClientTopic::GenerateDefaultVfsStorageContainerFileExt( void )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::StorageToPubSubRequest::SaveConfig( CORE::CDataNode & tree ) const
{
    return false;
}
...
bool
CTestPubSubClientTopic::StorageToPubSubRequest::LoadConfig( const CORE::CDataNode & treeroot )
{
    return false;
}
...
CTestPubSubClientTopic::GetClient( void )
{GUCEF_TRACE;
...
{GUCEF_TRACE;
    ...
    {
        TEventCallback callback( this, &CTestPubSubClientTopic::OnReconnectTimerCycle );
    ...
    {
        TEventCallback callback( this, &CTestPubSubClientTopic::OnSyncVfsOpsTimerCycle );
    ...
    {
        TEventCallback callback( this, &CTestPubSubClientTopic::OnBufferContentTimeWindowCheckCycle );
...
bool
CTestPubSubClientTopic::IsPublishingSupported( void )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::IsSubscribingSupported( void )
{GUCEF_TRACE;
...
const CORE::CString&
CTestPubSubClientTopic::GetTopicName( void ) const
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::Publish( CORE::UInt64& publishActionId  ,
...
bool
CTestPubSubClientTopic::Publish( TPublishActionIdVector& publishActionIds                   ,
...
template < typename T >
bool
CTestPubSubClientTopic::PublishViaMsgPtrs( TPublishActionIdVector& publishActionIds ,
                                              const std::vector< T >& msgs             ,
                                              bool notify                              )
{GUCEF_TRACE;

    if ( CTestPubSubClientTopicConfig::CHANNELMODE_PUBSUB_TO_TEST != m_config.mode )
    {
        GUCEF_WARNING_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic(" + CORE::PointerToString( this ) +
            "):PublishViaMsgPtrs: Attempt to publish messages on a topic that is not configured as CHANNELMODE_PUBSUB_TO_TEST" );
        return false;
    }
            ...
            if ( GUCEF_NULL == m_currentWriteBuffer )
            {
                ...
                if ( GUCEF_NULL == m_currentWriteBuffer )
                {
                    if ( m_config.performVfsOpsASync )
                    {
                        ...
                        GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "TestPubSubClientTopic(" + CORE::PointerToString( this ) +
            ...
            while ( i != msgs.end() )
            {
                ...
                if ( PUBSUB::CPubSubMsgBinarySerializer::Serialize( m_config.pubsubBinarySerializerOptions, *(*i), bufferOffset, *m_currentWriteBuffer, msgBytesWritten ) && 0 != msgBytesWritten )
                {
                    ...
                    if ( !bufferMetaData->msgOffsetIndex.empty() )
                    {
                        ...
                        {
                            GUCEF_ERROR_LOG( CORE::LOGLEVEL_BELOW_NORMAL, "TestPubSubClientTopic(" + CORE::PointerToString( this ) +
                    ...
                    GUCEF_DEBUG_LOG( CORE::LOGLEVEL_BELOW_NORMAL, "TestPubSubClientTopic(" + CORE::PointerToString( this ) +
    ...
    {
        GUCEF_EXCEPTION_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic(" + CORE::PointerToString( this ) + "):PublishViaMsgPtrs: exception: " + CORE::CString( e.what() ) );
        return false;
    }
}
...
void
CTestPubSubClientTopic::FinalizeWriteBuffer( TStorageBufferMetaData* bufferMetaData, CORE::UInt32 bufferOffset )
{GUCEF_TRACE;
    ...
    GUCEF_DEBUG_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic(" + CORE::PointerToString( this ) +
...
void
CTestPubSubClientTopic::FinalizeWriteBuffer( void )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::SetupToSubscribe( PUBSUB::CPubSubClientTopicConfig& config )
{GUCEF_TRACE;
...
CORE::CString
CTestPubSubClientTopic::GenerateMetricsFriendlyTopicName( const CORE::CString& topicName )
{GUCEF_TRACE;
...
CORE::CString
CTestPubSubClientTopic::ResolveVfsRootPath( void ) const
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::Disconnect( void )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::BeginVfsOps( void )
{GUCEF_TRACE;
    ...
    if ( !m_config.performVfsOpsASync )
    {
        ...
        if ( GUCEF_NULL != m_syncVfsOpsTimer )
        {
            GUCEF_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic(" + CORE::PointerToString( this ) + "):BeginVfsOps: Will use timer for sync VFS ops" );
...
bool
CTestPubSubClientTopic::StopVfsOps( void )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::AddStorageToPubSubRequest( const StorageToPubSubRequest& request )
{GUCEF_TRACE;

    if ( CTestPubSubClientTopicConfig::CHANNELMODE_TEST_TO_PUBSUB != m_config.mode )
    {
        GUCEF_WARNING_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic(" + CORE::PointerToString( this ) +
            "):PublishViaMsgPtrs: Attempt to source messages from storage on a topic that is not configured as CHANNELMODE_TEST_TO_PUBSUB" );
        return false;
    }
...
bool
CTestPubSubClientTopic::Subscribe( void )
{GUCEF_TRACE;

    MT::CScopeMutex lock( m_lock );
    if ( ( m_config.mode == TChannelMode::CHANNELMODE_TEST_TO_PUBSUB ) && ( m_config.autoPushAfterStartupIfStorageToPubSub ) )
    {
    ...
    GUCEF_SYSTEM_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:Subscribe: 'subscribe to topic' for Topic Name: " + m_config.topicName );
    return success;
}
...
bool
CTestPubSubClientTopic::SubscribeStartingAtMsgId( const CORE::CVariant& msgIdBookmark )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::SubscribeStartingAtMsgDateTime( const CORE::CDateTime& msgDtBookmark )
{GUCEF_TRACE;
    ...
    if ( ( m_config.mode == TChannelMode::CHANNELMODE_TEST_TO_PUBSUB ) && ( m_config.autoPushAfterStartupIfStorageToPubSub ) )
    {
        ...
        {
            GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:SubscribeStartingAtMsgDateTime: Failed to add publish request. DT=" + msgDtBookmark.ToIso8601DateTimeString( true, true ) + ", Topic Name: " + m_config.topicName );
            return false;
        }
    }
    ...
    if ( success )
    {
        GUCEF_SYSTEM_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:SubscribeStartingAtMsgDateTime: DT=" + msgDtBookmark.ToIso8601DateTimeString( true, true ) + ", Topic Name: " + m_config.topicName );
    }
    else
    {
        GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:SubscribeStartingAtMsgDateTime: Failed to start VFS Ops. DT=" + msgDtBookmark.ToIso8601DateTimeString( true, true ) + ", Topic Name: " + m_config.topicName );
    }
    return success;
}
...
bool
CTestPubSubClientTopic::SubscribeStartingAtBookmarkInfo( const TStorageBookmarkInfo& bookmarkInfo )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::SubscribeStartingAtBookmark( const PUBSUB::CPubSubBookmark& bookmark )
{GUCEF_TRACE;
    ...
    switch ( bookmark.GetBookmarkType() )
    {
        ...
        default:
        {
            GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:SubscribeStartingAtBookmark: Unsupported bookmark type given. Type=" + bookmark.GetBookmarkTypeName() );
            return false;
        }
    }
}
...
bool
CTestPubSubClientTopic::SyncBookmarkInfoToBookmark( const TStorageBookmarkInfo& info  ,
...
bool
CTestPubSubClientTopic::SyncBookmarkToBookmarkInfo( const PUBSUB::CPubSubBookmark& bookmark ,
...
CTestPubSubClientTopic::GetCurrentBookmark( void )
{GUCEF_TRACE;
...
bool 
CTestPubSubClientTopic::DeriveBookmarkFromMsg( const PUBSUB::CIPubSubMsg& msg    , 
...
bool
CTestPubSubClientTopic::AcknowledgeReceipt( const PUBSUB::CIPubSubMsg& msg )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::AcknowledgeReceipt( const PUBSUB::CPubSubBookmark& bookmark )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::AcknowledgeReceiptImpl( const PUBSUB::CPubSubBookmark& bookmark ,
                                                   CORE::UInt64 receiveActionId             )
{GUCEF_TRACE;

    MT::CScopeMutex lock( m_lock );

    const CTestPubSubClientConfig& clientConfig = m_client->GetConfig();
...
bool
CTestPubSubClientTopic::IsConnected( void ) const
{GUCEF_TRACE;
...
void
CTestPubSubClientTopic::UpdateIsHealthyStatus( bool newStatus )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::InitializeConnectivity( void )
{GUCEF_TRACE;
...
void
CTestPubSubClientTopic::OnReconnectTimerCycle( CORE::CNotifier* notifier    ,
...
void
CTestPubSubClientTopic::OnEndOfASyncVfsWork( void )
{GUCEF_TRACE;

    switch ( m_config.mode )
    {
        case TChannelMode::CHANNELMODE_TEST_TO_PUBSUB:
        {
            return;
        }
        case TChannelMode::CHANNELMODE_PUBSUB_TO_TEST:
        {
...
void
CTestPubSubClientTopic::PerformASyncVfsWork( void )
{GUCEF_TRACE;
...
void
CTestPubSubClientTopic::OnSyncVfsOpsTimerCycle( CORE::CNotifier* notifier    ,
                                                   const CORE::CEvent& eventId  ,
                                                   CORE::CICloneable* eventData )
{GUCEF_TRACE;
...
void
CTestPubSubClientTopic::OnPulseCycle( CORE::CNotifier* notifier    ,
                                         const CORE::CEvent& eventId  ,
                                         CORE::CICloneable* eventData )
{GUCEF_TRACE;
...
CTestPubSubClientTopic::TopicMetrics::TopicMetrics( void )
...
const CTestPubSubClientTopic::TopicMetrics&
CTestPubSubClientTopic::GetMetrics( void ) const
{GUCEF_TRACE;
...
const CORE::CString&
CTestPubSubClientTopic::GetMetricFriendlyTopicName( void ) const
{GUCEF_TRACE;
...
void
CTestPubSubClientTopic::OnBufferContentTimeWindowCheckCycle( CORE::CNotifier* notifier    ,
                                                                const CORE::CEvent& eventId  ,
                                                                CORE::CICloneable* eventData )
{GUCEF_TRACE;
...
void
CTestPubSubClientTopic::OnMetricsTimerCycle( CORE::CNotifier* notifier    ,
                                                const CORE::CEvent& eventId  ,
                                                CORE::CICloneable* eventData )
{GUCEF_TRACE;
...
const MT::CILockable*
CTestPubSubClientTopic::AsLockable( void ) const
{GUCEF_TRACE;
...
/*-------------------------------------------------------------------------*/
 ...
 bool
 CTestPubSubClientTopic::Lock( CORE::UInt32 lockWaitTimeoutInMs ) const
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::Unlock( void ) const
{GUCEF_TRACE;
...
CORE::CString
CTestPubSubClientTopic::GetPathToLastWrittenPubSubStorageFile( CORE::UInt32 lastOffset ) const
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::GetLastPersistedMsgAttributes( CORE::Int32 channelId          ,
...
bool
CTestPubSubClientTopic::GetPersistedBookmark( CORE::Int32 channelId              ,
                                                 const CORE::CString& topicName     ,
                                                 PUBSUB::CPubSubBookmark& bookmark )
{GUCEF_TRACE;
...
CTestPubSubClientTopic::GetSerializedMsgBuffers( void )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::LoadStorageFile( const CORE::CString& vfsPath       ,
...
{GUCEF_TRACE;
    ...
    if ( !m_config.decodeCodecFamily.IsNULLOrEmpty() && !m_config.decodeCodecName.IsNULLOrEmpty() )
    {
        ...
        {
            GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:LoadStorageFile: Cannot decode and load persisted file. CodeFamily:" + m_config.decodeCodecFamily +
        ...
        GUCEF_DEBUG_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:LoadStorageFile: Decoded and loaded persisted file. CodeFamily:" + m_config.decodeCodecFamily +
    ...
    else
    {
        ...
        {
            GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:LoadStorageFile: Cannot load last persisted file. VFS File: " + vfsPath );
            return false;
        }

        GUCEF_DEBUG_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:LoadStorageFile: Loaded VFS File: " + vfsPath );
    }
...
bool
CTestPubSubClientTopic::GetLastPersistedMsgAttributesWithOffset( CORE::Int32 channelId          ,
                                                                    const CORE::CString& topicName ,
                                                                    CORE::CVariant& msgId          ,
                                                                    CORE::CDateTime& msgDt         ,
...
{GUCEF_TRACE;
    ...
    if ( m_lastPersistedMsgId.IsNULLOrEmpty() && m_lastPersistedMsgDt == CORE::CDateTime::Empty )
    {
        ...
        if ( lastWrittenFilePath.IsNULLOrEmpty() )
        {
            ...
            GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:GetLastWrittenPubSubMsgId: Cannot obtain path to last written file with offset " + CORE::ToString( lastFileOffset ) );
            return false;
        }
        ...
        {
            fileExistedButHasIssue = false;
            GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:GetLastWrittenPubSubMsgId: Unable to load file from storage. Loading using last offset " + CORE::ToString( lastFileOffset ) );
            return false;
        }
        ...
        if ( 0 == lastStorageFileContent.GetDataSize() )
        {
            ...
            GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:GetLastPersistedMsgAttributes: last persisted file is empty. VFS File: " + lastWrittenFilePath );
            return false;
        }
        ...
        if ( !PUBSUB::CPubSubMsgContainerBinarySerializer::DeserializeMsgAtIndex( msg, true, lastStorageFileContent, 0, false, isCorrupted ) )
        {
            if ( isCorrupted )
            {
                ...
                GUCEF_WARNING_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:GetLastPersistedMsgAttributes: Failed to deserialize the last message, will attempt an index rebuild of the corrupt container" );
                ...
                {
                    GUCEF_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:GetLastPersistedMsgAttributes: Successfully performed an index rebuild of the corrupt container, discovered " + CORE::ToString( newRecoveredIndex.size() ) + " messages. Will attempt to add a new footer" );
                    ...
                    {
                        GUCEF_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:GetLastPersistedMsgAttributes: Successfully serialized a new footer to the previously corrupt container. Will attempt to persist the amended container" );
                        ...
                        if ( m_config.encodeCodecFamily.IsNULLOrEmpty() || m_config.encodeCodecName.IsNULLOrEmpty() )
                        {
                            ...
                            {
                                GUCEF_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:GetLastPersistedMsgAttributes: Successfully stored rebuild pub-sub message container at: " + lastWrittenFilePath );
                            }
                            else
                            {
                                fileExistedButHasIssue = true;
                                GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "TestPubSubClientTopic:GetLastPersistedMsgAttributes: StoreAsFile() Failed for rebuild message container" );
                                return false;
                            }
                        }
                        else
                        {
                            if ( vfs.EncodeAsFile( lastStorageFileContent, 0, lastWrittenFilePath, true, m_config.encodeCodecFamily, m_config.encodeCodecName ) )
                            {
                                ...
                                GUCEF_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:GetLastPersistedMsgAttributes: Successfully encoded and stored rebuild pub-sub message container resource at: \"" + lastWrittenFilePath + "\" with a encoded size ratio of " + CORE::ToString( m_encodeSizeRatio ) );
                            }
                            else
                            {
                                fileExistedButHasIssue = true;
                                GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "TestPubSubClientTopic:GetLastPersistedMsgAttributes: EncodeAsFile() Failed for rebuild message container" );
                                return false;
                            }
                        }
                    }
                ...
                if ( !PUBSUB::CPubSubMsgContainerBinarySerializer::DeserializeMsgAtIndex( msg, true, lastStorageFileContent, 0, false, isCorrupted ) )
                {
                    ...
                    fileExistedButHasIssue = true;
                    GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "TestPubSubClientTopic:GetLastPersistedMsgAttributes: Failed to load last message even after a successfull rebuild. isCorrupted=" + CORE::ToString( isCorrupted ) );
                    return false;
                }
            }
        }
...
bool
CTestPubSubClientTopic::GetTimestampsFromContainerFilename( const CORE::CString& fullPath     ,
                                                               CORE::CDateTime& firstMsgDt       ,
                                                               CORE::CDateTime& lastMsgDt        ,
...
{GUCEF_TRACE;
    ...
    GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:GetTimestampsFromContainerFilename: Failed to parse timestamps from filename " + segment );
    return false;
}
...
bool
CTestPubSubClientTopic::GetPathsToPubSubStorageFiles( const CORE::CDateTime& startDt  ,
...
void
CTestPubSubClientTopic::AddPublishActionIdsToNotify( const TPublishActionIdVector& publishActionIds, bool success )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::StoreNextReceivedPubSubBuffer( void )
{GUCEF_TRACE;
    ...
    if ( GUCEF_NULL != m_currentReadBuffer )
    {
        ...
        if ( m_config.encodeCodecFamily.IsNULLOrEmpty() || m_config.encodeCodecName.IsNULLOrEmpty() )
        {
            ...
            if ( vfs.StoreAsFile( vfsStoragePath, *m_currentReadBuffer, 0, true ) )
            {
                ...
                GUCEF_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:StoreNextReceivedPubSubBuffer: Successfully stored pub-sub mesage block at: " + vfsStoragePath );
            }
            else
            {
                GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "TestPubSubClientTopic:StoreNextReceivedPubSubBuffer: StoreAsFile() Failed" );
        ...
        else
        {
            ...
            if ( vfs.EncodeAsFile( *m_currentReadBuffer, 0, vfsStoragePath, true, m_config.encodeCodecFamily, m_config.encodeCodecName ) )
            {
                ...
                GUCEF_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:StoreNextReceivedPubSubBuffer: Successfully encoded and stored pub-sub mesage block at: \"" + vfsStoragePath + "\" with a encoded size ratio of " + CORE::ToString( m_encodeSizeRatio ) );
            }
            else
            {
                GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "TestPubSubClientTopic:StoreNextReceivedPubSubBuffer: EncodeAsFile() Failed" );
                AddPublishActionIdsToNotify( bufferMetaData->publishActionIds, false );
            }
        }
    }
...
void
CTestPubSubClientTopic::OnUnableToFullFillStorageToPubSubRequest( const StorageToPubSubRequest& failedRequest )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::LocateFilesForStorageToPubSubRequest( void )
{GUCEF_TRACE;
    ...
    while ( !m_stage1StorageToPubSubRequests.empty() )
    {
        ...
        GUCEF_DEBUG_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:LocateFilesForStorageToPubSubRequest: Request for messages in range " +
        ...
        {
            GUCEF_DEBUG_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:LocateFilesForStorageToPubSubRequest: Available data in the request range spans " +
        ...
        {
            GUCEF_WARNING_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:LocateFilesForStorageToPubSubRequest: Did not obtain any storage paths for time range " +
...
bool
CTestPubSubClientTopic::ProcessNextPubSubRequestRelatedFile( void )
{GUCEF_TRACE;
    ...
    if ( i != m_stage2StorageToPubSubRequests.end() )
    {
        ...
        while ( n != queuedRequest.vfsPubSubMsgContainersToPush.end() )
        {
            ...
            if ( GetTimestampsFromContainerFilename( (*n), containerFileFirstMsgDt, containerFileLastMsgDt, containerCaptureDt ) )
            {
                ...
                GUCEF_DEBUG_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:ProcessNextPubSubRequestRelatedFile: Parsed file path container start and end DateTimes. Start=" +
            ...
            {
                GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:ProcessNextPubSubRequestRelatedFile: Failed to parse start and/or end DateTime from file at path \"" +
            ...
            if ( GUCEF_NULL != m_currentWriteBuffer )
            {
                ...
                {
                    GUCEF_DEBUG_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:ProcessNextPubSubRequestRelatedFile: Loading the container and used a subset of its data to serve (part of) the request: " + bufferMetaData->relatedStorageFile );
                    if ( LoadStorageFile( (*n), *m_currentWriteBuffer ) )
                    {
                        ...
                        {
                            GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:ProcessNextPubSubRequestRelatedFile: Failed obtain messages from pubsub msg container at path \"" +
                                (*n) + "\". The file will be disregarded as invalid and skipped" );
                            queuedRequest.vfsPubSubMsgContainersToPush.erase( (*n) );
                            n = queuedRequest.vfsPubSubMsgContainersToPush.begin();
                            totalSuccess = false;
                        ...
                        if ( isCorrupted )
                        {
                            ...
                            GUCEF_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:ProcessNextPubSubRequestRelatedFile: Successfully rebuild index for corrupted pubsub msg container at path \"" +
                        ...
                        else
                        {
                            ...
                            GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:ProcessNextPubSubRequestRelatedFile: Failed update pubsub msg container footer in write buffer to reflect messages in the requested time range" );
                            queuedRequest.vfsPubSubMsgContainersToPush.erase( (*n) );
                            n = queuedRequest.vfsPubSubMsgContainersToPush.begin();
                            totalSuccess = false;
                            continue;
                    ...
                    {
                        GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:ProcessNextPubSubRequestRelatedFile: Failed load pubsub msg container at path \"" +
                            (*n) + "\". The file will be disregarded as invalid and skipped" );
                        queuedRequest.vfsPubSubMsgContainersToPush.erase( (*n) );
                        n = queuedRequest.vfsPubSubMsgContainersToPush.begin();
                        totalSuccess = false;
                ...
                {
                    GUCEF_DEBUG_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:ProcessNextPubSubRequestRelatedFile: Loading the entire container as-is to serve (part of) the request: " + bufferMetaData->relatedStorageFile );
                    if ( LoadStorageFile( (*n), *m_currentWriteBuffer ) )
                    {
            ...
            else
            {
                ...
                GUCEF_DEBUG_LOG( CORE::LOGLEVEL_BELOW_NORMAL, "TestPubSubClientTopic:ProcessNextPubSubRequestRelatedFile: No write buffer available" );
                return totalSuccess;
            }
            ++n;
        }
        ...
        {
            GUCEF_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic:ProcessNextPubSubRequestRelatedFile: Completed storage request. Start=" +
...
void
CTestPubSubClientTopic::OnStoredPubSubMsgTransmissionFailure( const CORE::CDateTime& firstMsgDt )
{GUCEF_TRACE;
...
CORE::UInt32
CTestPubSubClientTopic::GetMsgsWrittenToStorageCounter( bool resetCounter )
{GUCEF_TRACE;
...
CORE::UInt32
CTestPubSubClientTopic::GetMsgBytesWrittenToStorageCounter( bool resetCounter )
{GUCEF_TRACE;
...
CORE::UInt32
CTestPubSubClientTopic::GetMsgsLoadedFromStorageCounter( bool resetCounter )
{GUCEF_TRACE;
...
CORE::UInt32
CTestPubSubClientTopic::GetMsgBytesLoadedFromStorageCounter( bool resetCounter )
{GUCEF_TRACE;
...
CORE::UInt32
CTestPubSubClientTopic::GetStorageCorruptionDetectionCounter( bool resetCounter )
{GUCEF_TRACE;
...
CORE::UInt32
CTestPubSubClientTopic::GetStorageDeserializationFailuresCounter( bool resetCounter )
{GUCEF_TRACE;
...
bool
CTestPubSubClientTopic::TransmitNextPubSubMsgBuffer( void )
{GUCEF_TRACE;
    ...
    if ( !PUBSUB::CPubSubMsgContainerBinarySerializer::DeserializeWithRebuild( msgs, true, msgOffsetIndex, *m_currentReadBuffer, isCorrupted, m_config.bestEffortDeserializeIsAllowed ) )
    {
        ...
        GUCEF_ERROR_LOG( CORE::LOGLEVEL_NORMAL, "TestPubSubClientTopic(" + CORE::PointerToString( this ) +
