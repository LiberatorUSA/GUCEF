 ...
 */
//GUCEF_OSMAIN_BEGIN
GUCEF_OSSERVICEMAIN_BEGIN( "MsmqMetrics" )
{GUCEF_TRACE;
    ...
    CORE::CString logFilename = CORE::CombinePath( outputDir, "MsmqMetrics_log.txt" );
    ...
    if ( !msmqMetrics.LoadConfig( keyValueList, *globalConfig ) )
    {
        delete globalConfig;
        GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "MsmqMetrics: Exiting because LoadConfig failed" );
        return -1;
    }
    delete globalConfig;

    if ( !msmqMetrics.Start() )
    {
        GUCEF_ERROR_LOG( CORE::LOGLEVEL_CRITICAL, "MsmqMetrics: Failed to Start()" );
        return -2;
    }
    ...
    msmqMetrics.SetStandbyMode( true );

    }
    catch ( const std::exception& e )
    {
        GUCEF_EXCEPTION_LOG( CORE::LOGLEVEL_CRITICAL, "MsmqMetrics: Main thread stl exeption: " + CORE::CString( e.what() ) );
        //throw e;
    }
    #ifndef GUCEF_DEBUG_MODE
    catch ( ... )
    {
        GUCEF_EXCEPTION_LOG( CORE::LOGLEVEL_CRITICAL, "MsmqMetrics: Main thread unknown exeption occured. Caught in catch-all" );
        throw;
    }
    #endif
