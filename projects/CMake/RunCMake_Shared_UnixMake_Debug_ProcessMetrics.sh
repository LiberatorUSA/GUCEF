...
NIXMAKE_OUTPUTDIR="$OUTPUTDIR/UnixMakeDbg_ProcessMetrics"

echo "*** Invoking CMake ***"
echo "Output dir for this CMake target = $NIXMAKE_OUTPUTDIR"
echo "CMake will use source root: $MAINCMAKE"
cmake -DBUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=debug -G"Unix Makefiles" -H"$MAINCMAKE" -B$NIXMAKE_OUTPUTDIR
